...
double f1(double x[N]) {return (x[0] +cos(x[0]*x[1]*x[2]-1));}

double f2(double x[N]) {return (pow((1-x[0]), 1./4.) + x[1] + 0.05*pow(x[2], 2)-0.15*x[2] -1);}

double f3(double x[N]) {return (-pow(x[0], 2)-0.1*pow(x[1], 2)+x[2]-1);}
